...
read -p "Are you logged in to the govdotbd CF org above and targeting the correct space? (y/n) " -n 1 -r
echo
if [[ ! $REPLY =~ ^[Yy]$ ]]
then
...
while read -r username password; do
    gh secret --repo bdcert/getgov set CF_${upcase_space}_USERNAME --body $username
    gh secret --repo bdcert/getgov set CF_${upcase_space}_PASSWORD --body $password
done
