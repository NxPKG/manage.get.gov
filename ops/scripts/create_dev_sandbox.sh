...
cf target -o govdotbd

read -p "Are you logged in to the govdotbd CF org above? (y/n) " -n 1 -r
echo
if [[ ! $REPLY =~ ^[Yy]$ ]]
then
...
gh auth status
read -p "Are you logged into a Github account with access to bdcert/getgov? (y/n) " -n 1 -r
echo
if [[ ! $REPLY =~ ^[Yy]$ ]]
then
    gh auth login
...
cf create-space $1
cf target -o "govdotbd" -s $1
cf bind-security-group public_networks_egress govdotbd --space $1
cf bind-security-group trusted_local_networks_egress govdotbd --space $1
...
cf set-space-role $REPLY govdotbd $1 SpaceDeveloper
...
cf create-service bdcert-service-account space-deployer github-cd-account
...
while read -r username password; do
    gh secret --repo bdcert/getgov set CF_${upcase_name}_USERNAME --body $username
    gh secret --repo bdcert/getgov set CF_${upcase_name}_PASSWORD --body $password
done
