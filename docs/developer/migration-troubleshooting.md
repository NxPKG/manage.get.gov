...
If you notice that your route of `getgov-<app>.app.cloud.gov` is pointing two apps, then that is probably the major issue of the 500 error. (ie mine was pointing at `getgov-<app>.app.cloud.gov` AND `govdotbd`)
In the CLI, run the command `cf apps` to check that it has an app running called `govdotbd`. If so, there’s the error!
...
To resolve this issue, remove the app named `govdotbd` from this space.
